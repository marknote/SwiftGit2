...
class LocalGitServiceProvider {
    ...
    private func checkedSignature() throws -> Signature {
        if signature == nil {
            signature = Signature(name: "app", email: "app@ios")
        }
    ...
    private func load() {
        ...
        case .failure(let error):
            ...
            let initResult = Repository.create(at: workingURL)
            switch initResult {
            case let .success(repository):
                ...
                do {
                    ...
                    _ = try repository.commit(
                        tree: treeOid, parents: [], message: "init", signature: Signature(name: "", email: "")
                    ).get()
                } catch {
            ...
            case let .failure(error):
                print("Fail to init repo:\(error.localizedDescription) ")
            }
        }
    }
